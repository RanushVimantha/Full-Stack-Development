...
const TheaterListsByMovie = ({ movies, selectedMovieIndex, setSelectedMovieIndex, auth }) => {
	...
	return (
		<>
			<CinemaLists {...props} />
			<div className="mx-4 h-fit rounded-md bg-[#181818] text-gray-900 drop-shadow-md sm:mx-8">
				<div className="flex flex-col gap-6 p-4 sm:p-6">
					<DateSelector selectedDate={selectedDate} setSelectedDate={setSelectedDate} />
					<div className="flex flex-col gap-4 rounded-md bg-[#fff] py-4">
						<div className="flex items-center ">
							<img src={movies[selectedMovieIndex].img} className="w-32 px-4 drop-shadow-md" />
							<div>
								<h4 className="text-2xl font-bold">{movies[selectedMovieIndex].name}</h4>
